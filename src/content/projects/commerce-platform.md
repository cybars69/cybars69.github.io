...
blurb: "A multi-tenant storefront platform where every store gets its own Worker, plans are signed entitlements, and AI agents can run the shop over MCP."
era: recent
order: 3
image: /images/projects/commerce-platform.png
alt: "Diagram: merchant, control plane, per-store Worker, storefront, with signed entitlements and MCP for AI agents"
...
I wrote more about that in [plans that never block checkout](/blog/plans-that-never-block-checkout/).
...
- [Signed entitlements over KV: Ed25519, a five-step fallback chain, and a document that can only narrow](/blog/signed-entitlements-over-kv/)
- [The control-plane seam: idempotent usage snapshots, an event outbox and HMAC commands](/blog/control-plane-seam-usage-events-hmac/)
- [Demo mode as signed config, not a fork](/blog/demo-mode-as-signed-config/)
- [Two currencies, one invoice](/blog/two-currencies-one-invoice/)
- [Plans that never block checkout](/blog/plans-that-never-block-checkout/)
